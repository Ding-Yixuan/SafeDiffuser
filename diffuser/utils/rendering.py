...
class MazeRenderer:
    ...
    def renders(self, observations, conditions=None, title=None):
        ...
        x1 = (0.6/7) * np.cos(theta) + (2.5/7)
        y1 = (0.6/7) * np.sin(theta) + (2.5/7)
        ...
        x2 = (0.6/7) * np.sqrt(np.abs(np.cos(theta))) * np.sign(np.cos(theta)) + (4.5/7)
        y2 = (1.1/7) * np.sqrt(np.abs(np.sin(theta))) * np.sign(np.sin(theta)) + (5.0/7)
