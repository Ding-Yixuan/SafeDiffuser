...
class MazeRenderer:
    ...
    def renders(self, observations, conditions=None, title=None):
        ...
        x1 = (0.8/7) * np.cos(theta) + (2.5/7)
        y1 = (0.8/7) * np.sin(theta) + (2.5/7)
        ...
        x2 = (0.8/7) * np.sqrt(np.abs(np.cos(theta))) * np.sign(np.cos(theta)) + (4.5/7)
        y2 = (1.3/7) * np.sqrt(np.abs(np.sin(theta))) * np.sign(np.sin(theta)) + (5.0/7)
    ...
    def render_diffusion(self, savepath, diffusion_path, **video_kwargs):
        '''
            diffusion_path : [ n_diffusion_steps x horizon x joined_dim ]
        '''
        ...
        save_video(savepath, frame, **video_kwargs)
...
class Maze2dRenderer(MazeRenderer):
    ...
    def renders(self, observations, conditions=None, **kwargs):
        ...
        observations = observations + .5    #note offset
        if len(bounds) == 2:
            _, scale = bounds
            observations /= scale
        elif len(bounds) == 4:
